class Article < ActiveRecord::Base
  ...
  has_many :images, :as => :viewable, :class_name => 'ArticleImage', :order => :position, :dependent => :destroy
  ...
  def to_param
    ...
    def create_path
  	...
  	def path_exists?(new_path)
  		article = Article.find_by_path(new_path)
  		same_article = article == self
  		return (article != nil && !same_article)
  	end
